...
export function SkillLevelIcon({ level, className }: SkillLevelIconProps) {
  const levelData = LEVEL_DETAILS[level];

  if (!levelData) {
    return (
      <img
        src={`https://cdn.faceit.com/labels/skill-level-${level}.svg`}
        alt={`Level ${level}`}
        className={className}
      />
    );
  ...
  return (
    ...
    >
      ...
      <path
        d={BASE_GREY_PATH.d}
        fill={BASE_GREY_PATH.fill}
        fillOpacity={BASE_GREY_PATH.fillOpacity}
        fillRule="evenodd"
        clipRule="evenodd"
      />
      {levelData.paths.map((path, index) => (
        <path
          key={`level-${level}-path-${index}`}
          d={path.d}
          fill={path.fill}
